...
# 本地编译尝试,纯初学者
    my_branch 尝试编译刷入运行成功,但是配置环境安装的avbtool不是python因此我改了build_images.sh
    编译流程:
    ```
    source env.sh
    (optional)bash clean_source_tree.sj
    bash build.sh > build.log 2>&1 #防止出错直接退出wsl
    bash build_images_fix.sh > build_image.log 2>&1 #设置所有环境变量再运行build_images.sh
...
预置apatch,或者apatch修补过的boot.img的签名验证?是否要先修补再验证?